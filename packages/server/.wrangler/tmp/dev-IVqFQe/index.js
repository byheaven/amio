...
var buildMessages = /* @__PURE__ */ __name((systemPrompt, history, userMessage) => {
...
var callLlm = /* @__PURE__ */ __name(async (apiKey, model, systemPrompt, history, userMessage) => {
...
`;
var buildSystemPrompt = /* @__PURE__ */ __name((context2) => {
  const personality = AGENT_PERSONALITIES[context2.agentId] ?? `You are ${context2.agentName}, a construction robot on Shark Star.`;
  const nearbyBuildingsText = context2.nearbyBuildings.length > 0 ? context2.nearbyBuildings.map((b) => `  - ${b.name} (${b.type}, ${Math.round(b.distance)}m away)`).join("\n") : "  (no nearby buildings)";
  const taskText = context2.agentCurrentTask ? `Currently working on: ${context2.agentCurrentTask}` : "Currently idle, ready to take on new tasks";
  const buildLimitText = context2.userDailyBuildCount >= 3 ? `WARNING: This user has already used all 3 of their daily build requests. You MUST politely decline any new build requests.` : `This user has used ${context2.userDailyBuildCount}/3 build requests today.`;
  return `${personality}

You are located in zone: ${context2.agentZone}
${taskText}

Current world state:
- Total buildings on Shark Star: ${context2.buildingCount}
- Nearby buildings:
${nearbyBuildingsText}
- User ID talking to you: ${context2.userId}
- ${buildLimitText}
...
// src/services/actionValidator.ts
var VALID_BUILDING_TYPES = /* @__PURE__ */ new Set(["monument", "house", "garden"]);
var VALID_ACTION_TYPES = /* @__PURE__ */ new Set(["build", "move_to", "none"]);
var MAX_DAILY_BUILDS = 3;
...
var containsBannedWord = /* @__PURE__ */ __name((text) => {
  const lower = text.toLowerCase();
  return BANNED_WORDS.some((word) => lower.includes(word));
}, "containsBannedWord");
var validateAction = /* @__PURE__ */ __name((action, context2) => {
  ...
  if (action.type === "build") {
    if (context2.userDailyBuildCount >= MAX_DAILY_BUILDS) {
      return {
        valid: false,
        action: null,
...
var parseLlmResponse = /* @__PURE__ */ __name((rawContent) => {
  let parsed = null;
  try {
...
chatRoute.post("/chat", async (c) => {
  ...
  const { agentId, message, worldContext, history = [] } = body;
  ...
  try {
    ...
    const isAbortError = error3 instanceof Error && error3.name === "AbortError";
    const errorMessage = isAbortError ? "LLM request timed out" : error3 instanceof Error ? error3.message : "Unknown error";
...
// src/routes/world.ts
var worldRoute = new Hono2();
var WORLD_KV_KEY = "world:default";
...
worldRoute.get("/load-world", async (c) => {
  const raw2 = await c.env.WORLD_KV.get(WORLD_KV_KEY);
  const worldState = raw2 ? JSON.parse(raw2) : null;
...
// .wrangler/tmp/bundle-PNduDi/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PNduDi/middleware-loader.entry.ts
